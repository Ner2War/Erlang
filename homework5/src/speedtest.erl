-module(speedtest).
-export([testpd/0, testmap/0, testdict/0,testets/0,testprop/0,alltest/0]).
-define(COUNT, 100000).
...
testpd()->
    ...
    io:format("time needed to new put: ~p ~n ", [Time2]),
...
testdict() ->
    ...
    {Time2, Value2} = timer:tc(fun() ->
        dict:store(key1, "val 1", Value1)
    end),
    io:format("Time needed to add: ~p ~n ", [Time2]),
    {Time3, Value3} = timer:tc(fun() ->
        dict:store(key1, "new val", Value2) 
    end),
    io:format("Time needed to change value: ~p ~n ", [Time3]),
    {Time4, _Value4} = timer:tc(fun() ->
        dict:fetch(key1, Value3)
    end),
...
testets() ->
    {Time1, _Value1} = timer:tc(fun() ->
        ets:new(speed,[public,named_table])
    end),
    io:format("Time needed to create ETS: ~p ~n ", [Time1]),
    {Time2, _Value2} = timer:tc(fun() ->
        ets:insert(speed, {1, "Bob", 25, undefined})
    end),
    ...
    {Time4, _Value4} = timer:tc(fun() ->
        ets:insert(speed, {1, "Helen A.", 21, undefined})
    end),
    io:format("Time needed to change value: ~p ~n ", [Time4]),
    {Time5, _Value5} = timer:tc(fun() ->
        ets:match(speed, {'$1', '$2','$3', undefined})
    end),
...
testprop() ->
    ...
    io:format("Time needed to change value: ~p ~n ", [Time3]),
    {Time4,_Value4} = timer:tc(fun() ->
        proplists:get_value(key1, Value3)
    end),
    io:format("Time needed to get value: ~p ~n ", [Time4]),
    {Time5, Value5} = timer:tc(fun() ->
        [{_,_},{_,_},{_,_},{_,_},{_,_}]= Value2 
    end),
    io:format("Time needed to match: ~p ~n ", [Time5]),
    {Time6,_Value6} = timer:tc(fun() ->
        proplists:lookup(key1, Value5)
    end),
    ...
    {Time8,_Value8} = timer:tc(fun() ->
        proplists:get_value(5000, Value7)
    end),
